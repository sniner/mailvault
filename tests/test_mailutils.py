...
class TestMessageIdLedger:
    def test_an_archived_copy_can_be_claimed(self):
        ledger = mailutils.MessageIdLedger({"a@example.com": 1, "b@example.com": 1})
    ...
    def test_a_copy_is_claimed_only_once(self):
        ...
        ledger = mailutils.MessageIdLedger({"a@example.com": 1})
        assert ledger.take("a@example.com") is True
    ...
    def test_two_archived_copies_answer_twice(self):
        ...
        ledger = mailutils.MessageIdLedger({"a@example.com": 2})
        assert ledger.take("a@example.com") is True
        assert ledger.take("a@example.com") is True
        assert ledger.take("a@example.com") is False

    def test_empty_values_never_match(self):
        assert mailutils.MessageIdLedger({"a@example.com": 1}).take("") is False
        assert mailutils.MessageIdLedger({}).take("") is False

    def test_empty_values_are_not_counted(self):
        assert len(mailutils.MessageIdLedger({"a@example.com": 1, "": 3})) == 1

    def test_the_length_is_what_is_left_to_claim(self):
        ledger = mailutils.MessageIdLedger({"a@example.com": 2, "b@example.com": 1})
        assert len(ledger) == 3
    ...
    def test_truncated_long_id_matches_by_prefix(self):
        ...
        archived = _long_id()
        ledger = mailutils.MessageIdLedger({archived: 1})
    ...
    def test_short_id_is_never_prefix_matched(self):
        ...
        assert mailutils.MessageIdLedger({"abcdef@example.com": 1}).take("abc") is False

    def test_prefix_match_picks_the_right_entry(self):
        a, b = _long_id("aaa"), _long_id("bbb")
        ledger = mailutils.MessageIdLedger({a: 1, b: 1, "short@example.com": 1})
    ...
    def test_a_prefix_scan_passes_over_an_exhausted_entry(self):
        ...
        ledger = mailutils.MessageIdLedger({a: 1, b: 1})
