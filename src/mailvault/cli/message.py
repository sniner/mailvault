...
One command, two answers, and both rest on the same lookup: an id as the
reports print it, whole or begun, resolved by `jobs.retrieval` to the one entry
it names or refused for naming several.
"""
...
def export_entries(
    ...
    paths = [entry_path(store, one) for one in wanted]
